...
:::note[Перед началом]
**Урок 7 из 9 · 30–35 минут**

**Нужно знать:** массивы, функции и циклы. **После урока:** сможете хранить данные по ключам, считать частоты и находить повторяющиеся значения.
:::
...
Измените `countNumbers`, чтобы функция считала только положительные числа. Затем пройдите по полученному `Map` через `for...of` и выведите каждую пару `[число, количество]`.

## Самостоятельная практика: Contains Duplicate
